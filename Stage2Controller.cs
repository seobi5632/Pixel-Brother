...
public class Stage2Controller : StageManager
{
    ...
    void Update()
    {
        GameManager.instance.enmeyCntB = parentObj.transform.childCount;
    }
    void Spawn()
    {
        ...
        if (!GameManager.instance.isSpwan && !Check && GameManager.instance.isPlay && GameManager.instance.stage == 2)
        {
        ...
        if (Check && EndStage() && GameManager.instance.a)
        {
            ...
            if (GameManager.instance.play.GetComponent<CharacterStats>().currentHealth > 0 && !GameManager.instance.isPlay) 
                GameManager.instance.stage2 = true; //스테이지 잠금해제
